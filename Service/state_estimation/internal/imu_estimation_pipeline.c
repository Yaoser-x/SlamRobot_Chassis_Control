...
static void ImuEstimationPipeline_MapDevice(const bmi270_driver_state_t *source,
                                            state_estimation_imu_status_t *target)
{
    target->enabled             = source->enabled;
    target->online              = source->online;
    target->chip_id             = source->chip_id;
    target->last_error          = source->last_error;
    target->init_state          = source->init_state;
    ...
    target->drdy_count          = source->drdy_count;
    target->poll_fallback_count = source->poll_fallback_count;
    target->spi_error_count     = source->spi_error_count;
    target->init_failure_count  = source->init_failure_count;
    target->fifo_overflow_count = source->fifo_error_count;
    target->temperature_c       = source->temperature_c;
    target->temperature_valid   = source->temperature_valid;
}
                                   ...
                                   state_estimation_imu_status_t *status)
{
    ...
    ImuEstimationPipeline_MapDevice(device, status);
    ...
    {
        status->quality_flags |= STATE_ESTIMATION_IMU_QUALITY_TIMESTAMP_ERROR
    ...
    for (uint8_t axis = 0U; axis < 3U; ++axis)
    {
        ...
        {
            status->accel_g[axis] = ImuSignalFilter_LowPass(status->accel_g[axis], sensor_accel_g[axis], IMU_ACCEL_FILTER_ALPHA);
            status->body_accel_g[axis] = ImuSignalFilter_LowPass(status->body_accel_g[axis], body_accel_g[axis], IMU_ACCEL_FILTER_ALPHA);
            status->ros_accel_g[axis] = ImuSignalFilter_LowPass(status->ros_accel_g[axis], ros_accel_g[axis], IMU_ACCEL_FILTER_ALPHA);
            status->gyro_filtered_dps[axis] = ImuSignalFilter_LowPass(status->gyro_filtered_dps[axis], body_gyro_dps[axis], IMU_GYRO_FILTER_ALPHA);
            status->ros_gyro_dps[axis] = ImuSignalFilter_LowPass(status->ros_gyro_dps[axis], ros_gyro_dps[axis], IMU_GYRO_FILTER_ALPHA);
        }
...
{
    if (calibration_active != 0U || ImuBmi270GyroCalAccumulator_Begin(&calibration_accumulator, samples, interval_ms) == 0U)
    {
        return 0U;
    }
                                              ...
                                              state_estimation_imu_status_t *status)
{
    ...
    if (status->gyro_calibrated == 0U && calibration_active == 0U && stationary != 0U
        && (int32_t)(now_ms - auto_cal_next_ms) >= 0
        && status->gyro_auto_cal_attempts < IMU_AUTO_CAL_MAX_ATTEMPTS)
    {
        (void)ImuEstimationPipeline_BeginCalibration(500U, 10U, 1U);
    }
    if (calibration_active == 0U || status->sample_count == last_calibration_sample_count)
    {
        return;
    }
