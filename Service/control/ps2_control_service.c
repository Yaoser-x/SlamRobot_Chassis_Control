...
static void Ps2ControlService_SubmitCommand(float linear_x, float angular_z, uint32_t input_generation)
{
    ...
    (void)ControlService_SetCommandForGeneration(&cmd, input_generation);
}
...
static void Ps2ControlService_SubmitHeadingCommand(float angular_z)
{
    ...
    (void)ControlService_SetCommandForGeneration(&cmd, heading_motion_generation);
}
...
static uint32_t Ps2ControlService_ImuGateFlags(const imu_bmi270_state_t *imu_state, uint32_t now_ms)
{
    uint32_t flags = 0U;
...
static uint8_t Ps2ControlService_ImuUsable(const imu_bmi270_state_t *imu_state, uint32_t now_ms)
{
                                            ...
                                            uint32_t                  input_generation)
{
    ...
    if (Ps2ControlService_ImuUsable(imu_state, imu_now_ms) == 0U
        || input_generation != ControlService_GetMotionRevokeGeneration())
    {
        return 0U;
    }
    ...
    if (heading_motion_generation != ControlService_GetMotionRevokeGeneration())
    {
...
void Ps2ControlService_Init(void)
{
    ...
    heading_motion_generation = ControlService_GetMotionRevokeGeneration();
...
void Ps2ControlService_Update(void)
{
    ...
    uint32_t                    input_generation = ControlService_GetMotionRevokeGeneration();
    ...
    if (Ps2Hw_ReadSample(&sample) == 0U)
    {
        ...
        ControlService_ClearSource(CONTROL_SOURCE_PS2);
        return;
    }
    consecutive_read_failures = 0U;
    input_revoked             = (input_generation != ControlService_GetMotionRevokeGeneration()) ? 1U : 0U;
    ps2_state.rx_ok_count++;
    ImuBmi270_GetState(&imu_state);
    uint32_t imu_now_ms = PlatformTime_NowMs();

    linear_x  = -Ps2ControlService_NormalizeAxis(sample.left_y) * PS2_LINEAR_MAX_MPS;
    angular_z = -Ps2ControlService_NormalizeAxis(sample.right_x) * PS2_ANGULAR_MAX_RPS;
    ...
    {
        LineControlService_Enable((LineControlService_IsEnabled() == 0U) ? 1U : 0U);
    }
    ...
    if (input_revoked == 0U)
    {
        ...
        LineControlService_CalibrationGet(&cal_state);

        if ((pressed_btn2 & PS2_LINECAL_FLOOR_MASK) != 0U)
        {
            if (cal_state.collecting == 0U)
            {
                (void)LineControlService_CalibrationBegin(LINE_CALIBRATION_SURFACE_FLOOR, 100U);
        ...
        if ((pressed_btn2 & PS2_LINECAL_LINE_MASK) != 0U)
        {
            if (cal_state.collecting == 0U)
            {
                (void)LineControlService_CalibrationBegin(LINE_CALIBRATION_SURFACE_LINE, 100U);
                LedStatus_SetMode(LED_STATUS_CAL_RUNNING);
            }
        }
    ...
    linear_x  = Ps2ControlService_ClampFloat(linear_x, PS2_LINEAR_MAX_MPS);
    angular_z = Ps2ControlService_ClampFloat(angular_z, PS2_ANGULAR_MAX_RPS);
    ...
    if (ControlService_IsEmergencyStop() != 0U || ControlService_IsFaultStop() != 0U
        || ControlService_IsMaintenanceLocked() != 0U || input_revoked != 0U
        || (heading_control.active != 0U && heading_motion_generation != ControlService_GetMotionRevokeGeneration()))
    {
        if (heading_control.active != 0U)
        {
            RelativeYawControl_Cancel(&heading_control, RELATIVE_YAW_END_SAFETY_STOP);
    ...
    next_state.line_tracking_enabled   = LineControlService_IsEnabled();
    Ps2ControlService_CopyState(&ps2_state, &next_state);
