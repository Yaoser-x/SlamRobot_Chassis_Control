...
static safety_service_snapshot_t monitor_state;
...
static uint8_t SafetyService_CurrentBelowFaultThreshold(uint8_t motor, float current_a)
{
...
static uint8_t SafetyService_BatterySampleValid(const adc_monitor_state_t *adc_state)
{
...
static uint8_t SafetyService_TimeReached(uint32_t now_ms, uint32_t deadline_ms)
{
    return (((int32_t)(now_ms - deadline_ms)) >= 0) ? 1U : 0U;
}

static void SafetyService_UpdateOvercurrentCounters(const adc_monitor_state_t *adc_state,
                                                    ...
                                                    uint32_t                  *new_latched_flags)
{
    ...
    (void)ParamService_GetSnapshot(&params);
    debounce_count = (uint8_t)((params.current_fault_debounce_ms + CHASSIS_ADC_PERIOD_MS - 1U) / CHASSIS_ADC_PERIOD_MS);
...
static void SafetyService_UpdateCurrentDryRun(const adc_monitor_state_t *adc_state,
                                              const uint8_t              blanked[MOTOR_ID_COUNT])
{
    param_model_t params;
    uint8_t       debounce_count;
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        if (adc_state->current_a[i] > params.current_observe_a[i])
        {
            ...
            if (MOTOR_ADC_OVERCURRENT_FAULT_ENABLED == 0U || overcurrent_count[i] + 1U >= debounce_count)
            {
...
void SafetyService_Update(void)
{
    ...
    safety_service_snapshot_t next_state;
    ...
    TaskHealthService_UpdateTimeouts(now_ms);
    AdcMonitor_Update();
    ...
    estop_active      = ControlService_IsEmergencyStop();
    fault_stop_active = ControlService_IsFaultStop();
    active_source     = ControlService_GetActiveSource();
    TaskHealthService_GetHealth(&task_health);
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        else
        {
            ...
            motor_output_active[i] = 0U;
            if (SafetyService_TimeReached(now_ms, inactive_since_ms[i] + MOTOR_OVERCURRENT_STARTUP_REARM_MS) != 0U)
            {
                startup_blank_armed[i]        = 1U;
                overcurrent_blank_until_ms[i] = 0U;
            }
        ...
        overcurrent_blanked[i] =
            (startup_blank_armed[i] == 0U && SafetyService_TimeReached(now_ms, overcurrent_blank_until_ms[i]) == 0U)
                ? 1U
                : 0U;
    }

    SafetyService_UpdateCurrentDryRun(&adc_state, overcurrent_blanked);

    next_state                            = (safety_service_snapshot_t){0};
    ...
    next_state.task_timeout_mask          = TaskHealthService_GetTimeoutMask();
    for (uint8_t i = 0U; i < (uint8_t)TASK_HEALTH_SERVICE_COUNT; ++i)
    {
    ...
    SafetyService_UpdateOvercurrentCounters(&adc_state,
                                            overcurrent_blanked,
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        if (ChassisLayout_MotorEnabled((motor_id_t)i) != 0U && (mask & overcurrent_flags[i]) != 0U
            && (((snapshot.current_control_valid_mask & (uint8_t)(1U << i)) == 0U)
                || SafetyService_CurrentBelowFaultThreshold(i, snapshot.motor_current_a[i]) == 0U))
        {
