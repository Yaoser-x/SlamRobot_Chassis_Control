...
static void UpperUartService_HandleFrame(uint8_t cmd, const uint8_t *payload, uint8_t payload_len)
{
    if (cmd == UPPER_CMD_SET_VELOCITY)
    {
        ...
        if (UpperProtocol_ParseVelocityPayload(payload, payload_len, &velocity) != 0U)
        {
            ...
            ControlService_SetCommand(&chassis_cmd);
        }
    }
        ...
        case UPPER_RX_WAIT_BODY:
            ...
            if (upper_frame_index >= (uint8_t)(upper_frame_len + 1U))
            {
                ...
                if (checksum == expect)
                {
                    ...
                    UpperUartService_HandleFrame(cmd, payload, payload_len);
                    /* Record RX timestamp for OLED module online detection */
                    upper_last_rx_timestamp_ms      = PlatformTime_TaskNowMs();
...
static void UpperUartService_SendStatus(uint32_t now_ms)
{
    ...
    frame_len   = UpperProtocol_BuildFrame(UPPER_CMD_STATUS,
                                         upper_status_payload,
                                         payload_len,
                                         upper_tx_frame,
                                         sizeof(upper_tx_frame));
    if (frame_len > 0U)
    {
...
static void UpperUartService_SendDiagnostic(uint32_t now_ms)
{
    ...
    payload_len =
        UpperProtocol_BuildDiagnosticPayload(&diagnostic, upper_diagnostic_payload, sizeof(upper_diagnostic_payload));
...
static void UpperUartService_SendImuStatus(uint32_t now_ms)
{
    ...
    if (imu_state.online != 0U)
    {
        imu.status_flags |= UPPER_IMU_FLAG_ONLINE;
    }
    if (imu_state.gyro_calibrated != 0U)
    {
        imu.status_flags |= UPPER_IMU_FLAG_CALIBRATED;
    }
    if (imu_state.quality_flags != 0UL || imu_state.last_error != IMU_BMI270_ERROR_NONE)
    {
        imu.status_flags |= UPPER_IMU_FLAG_ERROR;
    }
    if (imu_state.sensor_time_valid != 0U)
    {
        imu.status_flags |= UPPER_IMU_FLAG_SENSOR_TIME;
    }
    imu.temperature_c = (int8_t)((imu_state.temperature_c >= 0.0f) ? (imu_state.temperature_c + 0.5f)
    ...
    frame_len   = UpperProtocol_BuildFrame(UPPER_CMD_IMU_STATUS,
                                         upper_imu_payload,
                                         payload_len,
                                         upper_imu_tx_frame,
                                         sizeof(upper_imu_tx_frame));
    if (frame_len > 0U)
    {
...
void UpperUartService_Update(void)
{
    ...
    UpperUartService_SendStatus(now_ms);
    UpperUartService_SendDiagnostic(now_ms);
    UpperUartService_SendImuStatus(now_ms);
}
