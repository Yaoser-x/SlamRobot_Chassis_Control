...
{
    uint8_t  cmd_len   = ROBOT_LINK_PROTOCOL_CMD_LEN(payload_len);
    uint16_t frame_len = (uint16_t)cmd_len + 4U;

    if (out == 0 || out_len < frame_len || payload_len > ROBOT_LINK_PROTOCOL_MAX_PAYLOAD)
    {
        return 0U;
    }
    ...
    out[0] = ROBOT_LINK_PROTOCOL_HEAD_0;
    out[1] = ROBOT_LINK_PROTOCOL_HEAD_1;
    out[2] = cmd_len;
    out[3] = cmd;
    if (payload_len > 0U)
    {
...
{
    if (payload == 0 || velocity == 0 || payload_len != ROBOT_LINK_PROTOCOL_VELOCITY_PAYLOAD_LEN)
    {
        return 0U;
    }
...
uint8_t UpperProtocol_BuildStatusPayload(const upper_status_payload_t *status, uint8_t *out, uint8_t out_len)
{
    ...
    if (status == 0 || out == 0 || out_len < ROBOT_LINK_PROTOCOL_STATUS_PAYLOAD_LEN)
    {
        return 0U;
    }

    out[offset++] = ROBOT_LINK_PROTOCOL_VERSION;
    ...
    for (uint8_t i = 0U; i < ROBOT_LINK_PROTOCOL_MOTOR_COUNT; ++i)
    {
...
uint8_t UpperProtocol_BuildImuStatusPayload(const upper_imu_status_payload_t *imu, uint8_t *out, uint8_t out_len)
{
    ...
    if (imu == 0 || out == 0 || out_len < ROBOT_LINK_PROTOCOL_IMU_STATUS_PAYLOAD_LEN)
    {
        return 0U;
    }
    ...
    return ROBOT_LINK_PROTOCOL_IMU_STATUS_PAYLOAD_LEN;
}

uint8_t
UpperProtocol_BuildDiagnosticPayload(const upper_diagnostic_payload_t *diagnostic, uint8_t *out, uint8_t out_len)
{
    if (diagnostic == 0 || out == 0 || out_len < ROBOT_LINK_PROTOCOL_DIAGNOSTIC_PAYLOAD_LEN)
    {
        return 0U;
    }

    out[0] = ROBOT_LINK_PROTOCOL_VERSION;
    out[1] = ROBOT_LINK_PROTOCOL_DIAGNOSTIC_SCHEMA_VERSION;
    ...
    return ROBOT_LINK_PROTOCOL_DIAGNOSTIC_PAYLOAD_LEN;
}
