...
_Static_assert((sizeof(reset_trace_record_t) % sizeof(uint32_t)) == 0, "reset trace record must stay word aligned");
...
static uint8_t PlatformResetTrace_StackFrameReadable(const uint32_t *stack)
{
...
static uint32_t PlatformResetTrace_EnterCritical(void)
{
    uint32_t primask = PlatformCritical_Enter();
    return primask;
}

static void PlatformResetTrace_ExitCritical(uint32_t primask)
{
    PlatformCritical_Exit(primask);
}

static reset_trace_task_t PlatformResetTrace_NormalizeTask(reset_trace_task_t task)
{
    return ((uint32_t)task <= (uint32_t)RESET_TRACE_TASK_DEBUG) ? task : RESET_TRACE_TASK_NONE;
}

static uint32_t PlatformResetTrace_CalcChecksum(const reset_trace_record_t *record)
{
...
static uint8_t PlatformResetTrace_RecordValid(const reset_trace_record_t *record)
{
    if (record->magic != RESET_TRACE_MAGIC || record->version != RESET_TRACE_VERSION)
    {
        return 0U;
    }
    return (record->checksum == PlatformResetTrace_CalcChecksum(record)) ? 1U : 0U;
}

static void PlatformResetTrace_PrepareLive(void)
{
    if (reset_trace_live.magic != RESET_TRACE_MAGIC || reset_trace_live.version != RESET_TRACE_VERSION)
    {
...
static void PlatformResetTrace_UpdateChecksum(void)
{
    reset_trace_live.checksum = PlatformResetTrace_CalcChecksum((const reset_trace_record_t *)&reset_trace_live);
}

static void PlatformResetTrace_ClearLive(void)
{
    reset_trace_live = (reset_trace_record_t){0};
}
...
{
    uint32_t primask = PlatformResetTrace_EnterCritical();
    task             = PlatformResetTrace_NormalizeTask(task);
    PlatformResetTrace_PrepareLive();
    ...
    PlatformResetTrace_UpdateChecksum();
    PlatformResetTrace_ExitCritical(primask);
}
...
void PlatformResetTrace_Capture(reset_trace_kind_t kind, uint32_t reason, uint32_t line)
{
    ...
    if (reset_trace_live.magic == RESET_TRACE_MAGIC && reset_trace_live.version == RESET_TRACE_VERSION)
    {
        task = PlatformResetTrace_NormalizeTask((reset_trace_task_t)reset_trace_live.task);
    }
    PlatformResetTrace_CaptureWithTask(kind, reason, line, task);
}
                                        ...
                                        reset_trace_task_t task)
{
    ...
    reset_trace_live.task    = (uint32_t)PlatformResetTrace_NormalizeTask(task);
...
void PlatformResetTrace_CaptureFaultStack(reset_trace_kind_t kind, const uint32_t *stack, uint32_t exc_return)
{
    ...
    if (PlatformResetTrace_StackFrameReadable(basic_frame) != 0U)
    {
...
uint8_t PlatformResetTrace_GetBootRecord(reset_trace_record_t *record)
{
    ...
    if (reset_trace_boot_captured == 0U)
    {
        ...
        PlatformResetTrace_ClearLive();
    }

    captured = PlatformResetTrace_RecordValid(&reset_trace_boot);
    if (record != 0)
    {
        if (captured != 0U)
        {
