...
{
    upper_uart_service_state_t state;
    ...
    UpperUartService_GetState(&state);
...
static void test_status_and_imu_share_async_priority_queue(void)
{
    ...
    require_int(tx_last_frame[4] == UPPER_PROTOCOL_VERSION, "diagnostic frame carries protocol version");
