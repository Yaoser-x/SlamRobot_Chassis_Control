...
static void feed_constant(line_calibration_t *calibration, line_calibration_surface_t surface, uint16_t base)
{
    uint16_t raw[LINE_CALIBRATION_CHANNELS];

    require_int(LineCalibration_Begin(calibration, surface, 4U) != 0U, "calibration collection starts");
    for (uint8_t sample = 0U; sample < 4U; ++sample)
    {
        ...
        LineCalibration_Feed(calibration, raw);
    }
}
...
static void test_black_line_white_floor_calibrates_active_low(void)
{
    ...
    LineCalibration_Init(&calibration);
    ...
    require_int(LineCalibration_Apply(&calibration, thresholds, &active_low) != 0U, "separated surfaces apply");
...
static void test_low_separation_is_rejected(void)
{
    ...
    require_int(LineCalibration_Apply(&calibration, thresholds, &active_low) == 0U, "ambiguous surfaces are rejected");
