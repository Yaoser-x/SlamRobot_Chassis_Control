...
static straight_line_controller_params_t DefaultParams(void)
{
    straight_line_controller_params_t params = {0};
...
static straight_line_controller_input_t DefaultInput(void)
{
    straight_line_controller_input_t input = {0};
    input.now_ms                           = 1000U;
    input.source                           = 2U;
    input.generation                       = 7U;
...
{
    straight_line_controller_t        controller;
    straight_line_controller_params_t params = DefaultParams();
    straight_line_controller_input_t  input  = DefaultInput();
    straight_line_controller_result_t result;

    StraightLineController_Init(&controller);
    result = StraightLineController_Step(&controller, &params, &input);
...
static void TestGyroPiAndReset(void)
{
    ...
    StraightLineController_Init(&controller);
    (void)StraightLineController_Step(&controller, &params, &input);
    input.now_ms += 100U;
...
static void TestClampAntiWindupAndSaturationAllocation(void)
{
    ...
    result                     = StraightLineController_Step(&controller, &params, &input);
    check_near(result.left_target_mps, -0.15f, 0.0001f, "reverse saturated side is not asked for more magnitude");
    check(result.right_target_mps > -0.15f, "reverse correction is converted to slowing the available side");
    ...
    result                     = StraightLineController_Step(&controller, &params, &input);
    check_near(result.heading_integral_deg_s, 0.0f, 0.0001f, "current limiting freezes heading integral");
}
