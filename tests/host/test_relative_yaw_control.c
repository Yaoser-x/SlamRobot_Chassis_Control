...
static void test_gyro_rate_integration_basic(void)
{
  ...
  require_int(RelativeYawControl_Start(&control, 90.0f, 0.0f, 100U, 4000U) != 0U,
              "relative yaw starts");
  ...
  require_close(control.accumulated_delta_deg, 2.0f, 0.001f,
                "200dps * 10ms = 2.0 deg");

  /* another 20ms at 200 deg/s = 4.0 deg more, total 6.0 */
  (void)RelativeYawControl_Update(&control, 0.0f, 200.0f, 130U, &angular_z);
  require_close(control.accumulated_delta_deg, 6.0f, 0.001f,
                "200dps * 30ms = 6.0 deg total");
}
...
static void test_full_turn_gyro_integration(void)
{
  ...
  require_close(control.accumulated_delta_deg, 360.0f, 0.01f,
                "10 * 100ms * 360dps = 360 deg");
}
...
static void test_output_limit_and_settle_window(void)
{
  ...
  /* 100ms 稳定应完成 */
  t += 1U;
  require_int(RelativeYawControl_Update(&control, 0.0f, 0.0f, t, &angular_z) == 0U,
              "100ms stable completes");
...
static void test_timeout_and_tick_wrap(void)
{
  ...
  require_int(RelativeYawControl_Update(&control, 0.0f, 0.0f, 4100U, &angular_z) == 0U,
...
static void test_large_dt_uses_fallback(void)
{
  ...
  require_close(control.accumulated_delta_deg, 2.0f, 0.01f,
                "large dt uses 20ms fallback: 100dps * 0.02s = 2.0 deg");
}
