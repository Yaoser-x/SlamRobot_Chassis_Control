...
static control_mode_snapshot_t        fake_control_mode;
...
static void SeedInputs(void)
{
    ...
    fake_control_mode                              = (control_mode_snapshot_t){0};
    ...
    fake_state.wheel.current_anomaly_mask          = (1U << 1);
    fake_state.wheel.anomaly_delivery_generation   = 7U;
    ...
    fake_safety.active_source                      = COMMAND_SOURCE_HOST;
    fake_control_mode.mode                         = CONTROL_MODE_LINE;
    fake_control_mode.generation                   = 3U;
...
int main(void)
{
    ...
    assert(snapshot.encoder.anomaly_generation == 7U);
    assert(snapshot.safety.battery_voltage == 12.3f);
    assert(snapshot.safety.motor_fault_mask == (1U << 1));
    assert(snapshot.safety.control_mode == CONTROL_MODE_LINE);
