...
{
    relative_heading_controller_t control;
    float                         angular_z = 0.0f;

    RelativeHeadingController_Init(&control);
    require_int(RelativeHeadingController_Start(&control, 90.0f, 0.0f, 100U, 4000U) != 0U, "relative yaw starts");

    /* 10ms at 200 deg/s = 2.0 deg */
    require_int(RelativeHeadingController_Update(&control, 0.0f, 200.0f, 110U, &angular_z) != 0U,
    ...
    (void)RelativeHeadingController_Update(&control, 0.0f, 200.0f, 130U, &angular_z);
...
static void test_full_turn_gyro_integration(void)
{
    ...
    RelativeHeadingController_Init(&control);
    (void)RelativeHeadingController_Start(&control, 360.0f, 0.0f, 0U, 10000U);

    /* 每 100ms 累积 36°, 10 步 = 360° */
    uint32_t t = 100U;
    for (int i = 0; i < 10; i++)
    {
        (void)RelativeHeadingController_Update(&control, 0.0f, 360.0f, t, &angular_z);
        t += 100U;
    }
...
static void test_output_limit_and_settle_window(void)
{
    ...
    RelativeHeadingController_Init(&control);
    (void)RelativeHeadingController_Start(&control, 90.0f, 0.0f, 100U, 4000U);

    /* 大误差 → 输出应达到上限 1.5 rad/s */
    (void)RelativeHeadingController_Update(&control, 0.0f, 0.0f, 110U, &angular_z);
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        t += 5U;
        (void)RelativeHeadingController_Update(&control, 0.0f, 890.0f, t, &angular_z);
    }
    require_close(control.accumulated_delta_deg, 89.0f, 0.1f, "accumulated reaches 89 deg");

    /* 高速率不应完成 */
    (void)RelativeHeadingController_Update(&control, 0.0f, 10.0f, t, &angular_z);
    t += 10U;
    require_int(control.active != 0U, "high yaw rate prevents completion");

    /* 低速率 + 小误差 → 进入 settling */
    (void)RelativeHeadingController_Update(&control, 0.0f, 0.0f, t, &angular_z);
    t += 10U;
    ...
    /* 100ms 稳定应完成 */
    t += 1U;
    require_int(RelativeHeadingController_Update(&control, 0.0f, 0.0f, t, &angular_z) == 0U, "100ms stable completes");
...
static void test_timeout_and_tick_wrap(void)
{
    ...
    RelativeHeadingController_Init(&control);
    (void)RelativeHeadingController_Start(&control, -90.0f, 0.0f, 100U, 4000U);
    require_int(RelativeHeadingController_Update(&control, 0.0f, 0.0f, 4099U, &angular_z) != 0U,
                "before timeout remains active");
    require_int(RelativeHeadingController_Update(&control, 0.0f, 0.0f, 4100U, &angular_z) == 0U,
                "timeout stops relative yaw");
    ...
    RelativeHeadingController_Init(&control);
    (void)RelativeHeadingController_Start(&control, 90.0f, 0.0f, 0xFFFFFFF0UL, 4000U);
    require_int(RelativeHeadingController_Update(&control, 0.0f, 0.0f, 0x00000010UL, &angular_z) != 0U,
...
static void test_large_dt_uses_fallback(void)
{
    ...
    (void)RelativeHeadingController_Update(&control, 0.0f, 100.0f, 601U, &angular_z);
