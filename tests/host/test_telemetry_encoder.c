...
int main(void)
{
    ...
    uint8_t                       payload_len;
    uint8_t                       actual[ROBOT_LINK_PROTOCOL_MAX_FRAME];
    uint8_t                       expected[ROBOT_LINK_PROTOCOL_MAX_FRAME];
    uint16_t                      actual_len;
    ...
    actual_len = TelemetryEncoder_BuildStatus(&snapshot, COMMUNICATION_LINK_ESP12F, actual, sizeof(actual));
    expected_len =
