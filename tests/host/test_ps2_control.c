...
void __disable_irq(void) { fake_primask = 1U; }
void __set_PRIMASK(uint32_t value) { fake_primask = value; }
uint32_t osKernelGetTickCount(void) { return fake_tick_ms; }
uint32_t HAL_GetTick(void) { return fake_hal_tick_ms; }
...
uint8_t Ps2Hw_IsAnalogMode(uint8_t mode) { return (mode == 0x73U) ? 1U : 0U; }
...
uint32_t ControlManager_GetMotionRevokeGeneration(void) { return fake_motion_revoke_generation; }
...
static void test_heading_imu_freshness_uses_hal_tick_epoch(void)
{
  ...
  require_int(state.heading_active != 0U,
              "fresh HAL-timestamped IMU starts heading despite RTOS epoch offset");
}
...
static void test_center_submits_zero_or_yields_to_line(void)
{
  ...
  require_int(last_command.linear_x == 0.0f && last_command.angular_z == 0.0f,
              "centered online PS2 command is zero");
...
static void test_offline_and_safety_stop_cancel_heading(void)
{
  ...
  Ps2Control_Update();
  Ps2Control_GetState(&state);
  require_int(state.online == 0U && state.heading_active == 0U,
              "three failed reads cancel heading and mark offline");
...
static void test_revoke_generation_cancels_heading_after_short_safety_event(void)
{
  ...
  Ps2Control_GetState(&state);
  require_int(state.heading_active == 0U &&
              state.heading_end_reason == RELATIVE_YAW_END_SAFETY_STOP,
...
static void test_heading_uses_generation_from_input_sample(void)
{
  ...
  Ps2Control_GetState(&state);
  require_int(state.heading_active == 0U,
              "maintenance crossing after input sample invalidates macro edge");
}
