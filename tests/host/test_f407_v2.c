...
static void test_status_v2_payload_layout_and_saturation(void)
{
  ...
  status.status_flags = UPPER_STATUS_FLAG_ESTOP |
                        UPPER_STATUS_FLAG_LINE_ENABLED |
...
{
  require_close(EncoderMath_CountDeltaSpeedMps(800,
                                               500U,
                                               2464.0f,
                                               0.035f),
                0.142799f,
...
static void test_imu_state_contract(void)
{
  ...
  require_int((sizeof(state.gyro_corrected_dps) / sizeof(state.gyro_corrected_dps[0])) == 3U, "gyro corrected axis count");
  require_int((sizeof(state.gyro_filtered_dps) / sizeof(state.gyro_filtered_dps[0])) == 3U, "gyro filtered axis count");
