...
static void test_tim1_break_latches_until_safe_manual_clear(void)
{
    ...
    require_int(tim1_instance.CCR1 == 0U && tim1_instance.CCR2 == 0U && tim1_instance.CCR3 == 0U
                    && tim1_instance.CCR4 == 0U,
...
static void test_pre_wake_tim1_break_is_cleared_after_inputs_stabilize(void)
{
    ...
    require_int(state.startup_qualified != 0U, "stable BKIN and nFAULT inputs qualify startup");
}
...
static void test_startup_low_bkin_latches_break_and_keeps_outputs_disabled(void)
{
    ...
    MotorDriver_Init();
    MotorDriver_GetState(&state);
    require_int(state.tim1_break_latched != 0U, "BKIN held low through startup qualification latches break");
    require_int(state.startup_qualified == 0U, "low BKIN cannot qualify startup");
    ...
    require_int((tim1_instance.BDTR & TIM_BDTR_MOE) == 0U, "failed startup qualification keeps main output disabled");
}
