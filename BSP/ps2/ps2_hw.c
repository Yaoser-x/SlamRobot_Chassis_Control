...
static uint8_t Ps2Hw_Probe(uint8_t require_analog)
{
    ...
    {
        if (Ps2Hw_Send(ps2_poll_frame, rx, (uint8_t)sizeof(rx)) != 0U && Ps2Hw_HandshakeOk(rx) != 0U
...
uint8_t Ps2Hw_ReadSample(ps2_hw_sample_t *sample)
{
    ...
    if (Ps2Hw_Send(ps2_poll_frame, rx, (uint8_t)sizeof(ps2_poll_frame)) == 0U || Ps2Hw_HandshakeOk(rx) == 0U)
    {
        return 0U;
    }
