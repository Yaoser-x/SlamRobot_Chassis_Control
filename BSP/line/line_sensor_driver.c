...
/* 监控/调试状态 */
static line_sensor_driver_state_t line_state;
...
static uint8_t LineSensorDriver_ComputeChecksum(const uint8_t *buf, uint8_t data_len)
{
    uint16_t sum = 0U;
...
static void LineSensorDriver_ResetParser(void)
{
...
static void LineSensorDriver_ClearUartFlags(void)
{
...
static void LineSensorDriver_RestartRxDma(uint8_t count_restart)
{
    (void)HAL_UART_DMAStop(&huart4);
    LineSensorDriver_ClearUartFlags();
    line_rx_read_pos = 0U;
    LineSensorDriver_ResetParser();
...
static void LineSensorDriver_SendByteAsync(const uint8_t *cmd)
{
    if (line_tx_busy != 0U)
    {
...
static void LineSensorDriver_ProcessByte(uint8_t byte)
{
    line_state.rx_bytes++;
    ...
    switch (line_rx_state)
    {
        ...
        case LINE_RX_WAIT_CHECKSUM:
        {
            ...
            uint8_t expected          = LineSensorDriver_ComputeChecksum(line_frame_buf, line_data_len);
...
void LineSensorDriver_Init(void)
{
    ...
    line_tx_busy     = 0U;
    LineSensorDriver_ResetParser();
    line_state       = (line_sensor_driver_state_t){0};
...
void LineSensorDriver_Update(void)
{
    ...
    {
        LineSensorDriver_ProcessByte(line_rx_dma_buffer[line_rx_read_pos]);
        line_rx_read_pos++;
...
void LineSensorDriver_InitSensor(void)
{
   ...
   *       之后用 LineSensorDriver_RequestAnalog() 发送 0x02 请求 21 字节模拟量帧。
   * 参考：HiWonder Arduino UART 例程
   */
    LineSensorDriver_SendByteAsync(&line_manual_mode_cmd);
}

void LineSensorDriver_RequestAnalog(void)
{
    /* 发送 0x02 请求模拟量帧，传感器返回 21 字节: 0x55 0xAA 0x02 0x10 CH1..CH8 CHECKSUM */
    LineSensorDriver_SendByteAsync(&line_analog_query_cmd);
}
...
void LineSensorDriver_OnUartError(void)
{
    ...
    LineSensorDriver_RestartRxDma(1U);
}
