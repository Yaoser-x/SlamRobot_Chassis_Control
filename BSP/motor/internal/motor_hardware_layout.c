...
static uint8_t MotorHardwareLayout_IsValidMotor(motor_id_t motor)
{
...
{
    if (MotorHardwareLayout_IsValidMotor(motor) == 0U)
    {
        return 0U;
    }
...
int8_t MotorHardwareLayout_MotorDirection(motor_id_t motor)
{
    if (MotorHardwareLayout_IsValidMotor(motor) == 0U || motor_layout[(uint32_t)motor].motor_direction < 0)
    {
        return -1;
    }
    return 1;
...
{
    if (MotorHardwareLayout_IsValidMotor(motor) == 0U || motor_layout[(uint32_t)motor].encoder_direction < 0)
    {
        return -1;
    }
    return 1;
...
uint8_t MotorHardwareLayout_SideMotorCount(motor_side_t side)
{
    uint8_t count = 0U;
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        if (MotorHardwareLayout_MotorEnabled((motor_id_t)i) != 0U
            && MotorHardwareLayout_MotorSide((motor_id_t)i) == side)
        {
            count++;
        }
    }
...
uint8_t MotorHardwareLayout_HasBothSides(void)
{
    return (MotorHardwareLayout_SideMotorCount(MOTOR_SIDE_LEFT) != 0U
            && MotorHardwareLayout_SideMotorCount(MOTOR_SIDE_RIGHT) != 0U)
               ? 1U
               : 0U;
}
