...
static int8_t MotorReversalStateMachine_Sign(int16_t value)
{
    if (value > 0)
    {
        return 1;
...
static uint16_t MotorReversalStateMachine_Abs(int16_t value)
{
    return (value < 0) ? (uint16_t)(-value) : (uint16_t)value;
}

static uint16_t
MotorReversalStateMachine_Ramp(uint16_t current, uint16_t target, uint16_t rise_step, uint16_t fall_step)
{
    if (current < target)
    {
...
static motor_reversal_output_t MotorReversalStateMachine_Output(const motor_reversal_state_machine_t *state,
...
{
    MotorReversalStateMachine_Disable(state);
}
...
void MotorReversalStateMachine_Disable(motor_reversal_state_machine_t *state)
{
    if (state != 0)
    {
        *state = (motor_reversal_state_machine_t){
            .current_ph_dir = -1,
                                                       ...
                                                       const motor_reversal_input_t   *input)
{
    ...
    target_dir           = MotorReversalStateMachine_Sign(input->requested_pwm);
    target_mag           = MotorReversalStateMachine_Abs(input->requested_pwm);
    applied_dir          = MotorReversalStateMachine_Sign(state->applied_pwm);
    applied_mag          = MotorReversalStateMachine_Abs(state->applied_pwm);

    if (target_dir == 0)
    {
        next_mag =
            MotorReversalStateMachine_Ramp(applied_mag, 0U, config->rise_step_per_cycle, config->fall_step_per_cycle);
        ...
        return MotorReversalStateMachine_Output(state, 0U);
    }
    ...
    if (state->applied_pwm == 0 && state->current_ph_dir != target_dir)
    {
        ...
        {
            return MotorReversalStateMachine_Output(state, 1U);
        }
    ...
    state->phase_initialized = 1U;
    next_mag                 = MotorReversalStateMachine_Ramp(MotorReversalStateMachine_Abs(state->applied_pwm),
                                              target_mag,
    ...
    return MotorReversalStateMachine_Output(state, phase_changed);
}
