...
static wheel_encoder_state_t  published_encoder_state;
...
static float WheelEncoderDriver_AbsF(float value)
{
    return (value < 0.0f) ? -value : value;
}

static int32_t WheelEncoderDriver_AbsI32(int32_t value)
{
...
float WheelEncoderDriver_CountDeltaSpeedMps(int32_t delta, uint32_t dt_ms, float counts_per_rev, float wheel_radius_m)
{
    return WheelSpeedEstimator_CountDeltaSpeedMps(delta, dt_ms, counts_per_rev, wheel_radius_m);
}
...
void WheelEncoderDriver_Init(void)
{
    ...
    published_encoder_state = (wheel_encoder_state_t){0};
    last_update_ms          = 0U;
    has_last_update         = 0U;
}
...
void WheelEncoderDriver_Update(uint32_t now_ms, const wheel_encoder_driver_config_t *config)
{
    ...
    for (uint32_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        {
            delta[i] = DirectionApply_Signed(WheelEncoderDriver_DiffCount(now_count[i], last_count[i], period),
    ...
    for (uint32_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        if (MotorHardwareLayout_MotorSide((motor_id_t)i) == MOTOR_SIDE_LEFT)
        {
            left_count++;
    ...
    for (uint32_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        if (MotorHardwareLayout_MotorSide((motor_id_t)i) == MOTOR_SIDE_LEFT && encoder_state.speed_valid[i] == 0U)
        {
            encoder_state.left_speed_valid = 0U;
        }
        if (MotorHardwareLayout_MotorSide((motor_id_t)i) == MOTOR_SIDE_RIGHT && encoder_state.speed_valid[i] == 0U)
        {
    ...
    if (left_count >= 2U)
    {
        ...
        {
            if (MotorHardwareLayout_MotorEnabled((motor_id_t)i) != 0U
                && MotorHardwareLayout_MotorSide((motor_id_t)i) == MOTOR_SIDE_LEFT)
            {
                if (found == 0U)
                {
        ...
        if (found >= 2U)
        {
            if (WheelEncoderDriver_AbsF(left_a_speed - left_b_speed) > CHASSIS_ENCODER_SIDE_SPEED_DIFF_MPS)
            {
                encoder_state.side_consistency_flags |= ENCODER_SIDE_CONSISTENCY_LEFT_SPEED;
            }
            if (WheelEncoderDriver_AbsI32(left_a_count - left_b_count) > CHASSIS_ENCODER_SIDE_COUNT_DIFF)
            {
    ...
    if (right_count >= 2U)
    {
        ...
        {
            if (MotorHardwareLayout_MotorEnabled((motor_id_t)i) != 0U
                && MotorHardwareLayout_MotorSide((motor_id_t)i) == MOTOR_SIDE_RIGHT)
            {
                if (found == 0U)
                {
        ...
        if (found >= 2U)
        {
            if (WheelEncoderDriver_AbsF(right_a_speed - right_b_speed) > CHASSIS_ENCODER_SIDE_SPEED_DIFF_MPS)
            {
                encoder_state.side_consistency_flags |= ENCODER_SIDE_CONSISTENCY_RIGHT_SPEED;
            }
            if (WheelEncoderDriver_AbsI32(right_a_count - right_b_count) > CHASSIS_ENCODER_SIDE_COUNT_DIFF)
            {
