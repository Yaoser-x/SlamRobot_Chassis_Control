...
static void FlashParam_MigrateV1Params(const param_store_v1_t *old, param_store_t *params)
{
...
static void FlashParam_MigrateV2Bundle(const flash_param_bundle_v2_t *old, flash_param_bundle_t *bundle)
{
...
static void FlashParam_MigrateV3Bundle(const flash_param_bundle_v3_t *old, flash_param_bundle_t *bundle)
{
...
FlashParam_EncodeBundle(const flash_param_bundle_t *bundle, uint32_t sequence, uint8_t *image, size_t image_size)
{
    ...
    encoded.crc32         = FlashParam_Crc32((const uint8_t *)&encoded, offsetof(flash_param_image_t, crc32));
...
FlashParam_DecodeBundle(const uint8_t *image, size_t image_size, flash_param_bundle_t *bundle, uint32_t *sequence)
{
    ...
    if (encoded->magic == FLASH_PARAM_MAGIC && encoded->schema_version == 2UL)
    {
        ...
        old_crc = FlashParam_Crc32((const uint8_t *)old, offsetof(flash_param_image_v2_t, crc32));
    ...
    if (encoded->magic == FLASH_PARAM_MAGIC && encoded->schema_version == 3UL)
    {
        ...
        old_crc = FlashParam_Crc32((const uint8_t *)old, offsetof(flash_param_image_v3_t, crc32));
        if (old_crc != old->crc32)
        {
            return FLASH_PARAM_STATUS_CRC_ERROR;
        }
    ...
    crc = FlashParam_Crc32((const uint8_t *)encoded, offsetof(flash_param_image_t, crc32));
...
static flash_param_status_t
FlashParam_DecodeLegacy(const uint8_t *image, size_t image_size, flash_param_bundle_t *bundle)
{
    ...
    if (legacy->magic != FLASH_PARAM_MAGIC || legacy->version != 1UL || legacy->data_size != sizeof(param_store_v1_t))
    {
        return FLASH_PARAM_STATUS_UNSUPPORTED;
    }
...
flash_param_status_t FlashParamHost_SeedSchema2WithCalibration(const param_store_t            *params,
                                                               const imu_bmi270_calibration_t *calibration)
{
    ...
    image.crc32         = FlashParam_Crc32((const uint8_t *)&image, offsetof(flash_param_image_v2_t, crc32));
...
flash_param_status_t FlashParamHost_SeedSchema3(const param_store_t *params)
{
    ...
    ImuBmi270Calibration_Default(&image.payload.imu_calibration);
    image.crc32         = FlashParam_Crc32((const uint8_t *)&image, offsetof(flash_param_image_v3_t, crc32));
    image.commit_marker = FLASH_PARAM_COMMIT_MARKER;
    memset(flash_param_host_slots[0], 0xFF, FLASH_PARAM_IMAGE_SIZE);
    memcpy(flash_param_host_slots[0], &image, sizeof(image));
    return FLASH_PARAM_STATUS_OK;
...
static flash_param_status_t FlashParam_WatchdogExitMaintenance(void)
{
    hiwdg.Init.Prescaler = flash_param_watchdog_saved_prescaler;
    hiwdg.Init.Reload    = flash_param_watchdog_saved_reload;
    return (HAL_IWDG_Init(&hiwdg) == HAL_OK) ? FLASH_PARAM_STATUS_OK : FLASH_PARAM_STATUS_WRITE_ERROR;
}
...
static flash_param_status_t FlashParam_StorageUnlock(void)
{
    ...
    return (HAL_FLASH_Unlock() == HAL_OK) ? FLASH_PARAM_STATUS_OK : FLASH_PARAM_STATUS_WRITE_ERROR;
}
...
static flash_param_status_t FlashParam_EraseSlot(uint8_t slot)
{
    ...
    erase.Sector       = (slot == 0U) ? FLASH_PARAM_STORAGE_SECTOR_A : FLASH_PARAM_STORAGE_SECTOR_B;
    erase.NbSectors    = 1U;
    erase.VoltageRange = FLASH_VOLTAGE_RANGE_3;
    hal_status         = HAL_FLASHEx_Erase(&erase, &sector_error);
    return (hal_status == HAL_OK && sector_error == 0xFFFFFFFFUL) ? FLASH_PARAM_STATUS_OK
                                                                  : FLASH_PARAM_STATUS_WRITE_ERROR;
}

static flash_param_status_t FlashParam_ProgramWord(uint8_t slot, size_t offset, uint32_t word)
{
    uint32_t address = ((slot == 0U) ? FLASH_PARAM_STORAGE_ADDR_A : FLASH_PARAM_STORAGE_ADDR_B) + (uint32_t)offset;
    return (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, address, word) == HAL_OK) ? FLASH_PARAM_STATUS_OK
                                                                                : FLASH_PARAM_STATUS_WRITE_ERROR;
}
#endif
...
flash_param_status_t FlashParam_LoadBundle(flash_param_bundle_t *bundle)
{
    ...
    legacy_status = FlashParam_DecodeLegacy(FlashParam_SlotData(1U), FLASH_PARAM_IMAGE_SIZE, bundle);
