...
static uint16_t                 adc_dma_buffer[POWER_ADC_DRIVER_CHANNEL_COUNT];
static uint32_t                 adc_update_period_ms;
static volatile uint16_t        adc_sample_snapshot[POWER_ADC_DRIVER_CHANNEL_COUNT];
...
static power_adc_driver_state_t adc_state;
...
static float PowerAdcDriver_RawToVoltage(uint16_t raw)
{
    return ((float)raw * POWER_ADC_DRIVER_VREF_V) / POWER_ADC_DRIVER_RESOLUTION_COUNTS;
}

static float PowerAdcDriver_RawDeltaToCurrentForMotor(motor_id_t motor, float raw_delta)
{
    ...
    return (raw_delta * POWER_ADC_DRIVER_VREF_V) / (POWER_ADC_DRIVER_RESOLUTION_COUNTS * scale);
}

static float PowerAdcDriver_Sqrtf(float value)
{
    float estimate;
...
static uint16_t PowerAdcDriver_ClampU16(uint32_t value)
{
    return (value > 65535UL) ? 65535U : (uint16_t)value;
}

static float PowerAdcDriver_FilterBattery(float previous, float sample)
{
    float alpha = POWER_ADC_DRIVER_BATTERY_FILTER_ALPHA;
...
static void PowerAdcDriver_ResetWindowAccumulators(void)
{
...
static void PowerAdcDriver_ResetCurrentZeroAccumulators(void)
{
...
static void PowerAdcDriver_UpdateCurrentZero(const uint32_t raw_sum[MOTOR_ID_COUNT],
                                             const uint16_t raw_min[MOTOR_ID_COUNT],
                                             const uint16_t raw_max[MOTOR_ID_COUNT],
                                             uint32_t       sample_count)
{
    if (POWER_ADC_DRIVER_CALIBRATION_ENABLED == 0U || current_zero_valid != 0U || current_zero_stationary == 0U)
    {
        return;
    }
    if (sample_count == 0UL)
...

    {
        ...
        if (total_count >= POWER_ADC_DRIVER_CURRENT_ZERO_SAMPLES)
        {
            for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
            {
            ...
            current_zero_sample_count = POWER_ADC_DRIVER_CURRENT_ZERO_SAMPLES;
...
static void PowerAdcDriver_ComputeCurrentWindow(uint32_t   delta_sum,
                                                ...
                                                float     *trimmed_a)
{
    ...
    rms_delta         = PowerAdcDriver_Sqrtf((float)delta_sq_sum / (float)sample_count);
    signed_mean_delta = (float)signed_delta_sum / (float)sample_count;
    {
        ...
        noise_delta          = PowerAdcDriver_Sqrtf(signed_sq_mean - signed_mean_sq);
    }
    if (sample_count > 2UL)
    {
    ...
    *mean_a        = PowerAdcDriver_RawDeltaToCurrentForMotor(motor, mean_delta);
    *rms_a         = PowerAdcDriver_RawDeltaToCurrentForMotor(motor, rms_delta);
    *peak_a        = PowerAdcDriver_RawDeltaToCurrentForMotor(motor, (float)delta_max);
    *signed_mean_a = PowerAdcDriver_RawDeltaToCurrentForMotor(motor, signed_mean_delta);
    *noise_a       = PowerAdcDriver_RawDeltaToCurrentForMotor(motor, noise_delta);
    *trimmed_a     = PowerAdcDriver_RawDeltaToCurrentForMotor(motor, trimmed_delta);
}

static float PowerAdcDriver_FilterCurrent(float previous, float sample)
{
...
{
    adc_state = (power_adc_driver_state_t){0};
    for (uint8_t i = 0U; i < POWER_ADC_DRIVER_CHANNEL_COUNT; ++i)
    {
    ...
    PowerAdcDriver_ResetWindowAccumulators();
    ...
    current_zero_sample_count  = 0U;
    if (HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adc_dma_buffer, POWER_ADC_DRIVER_CHANNEL_COUNT) == HAL_OK)
    {
...
void PowerAdcDriver_Update(void)
{
    ...
    static power_adc_driver_state_t next_state;
    ...
    if (samples_ready != 0U)
    {
        PowerAdcDriver_UpdateCurrentZero(raw_sum, raw_min, raw_max, raw_sample_count);
    }
    ...
    next_state.battery_voltage =
        PowerAdcDriver_FilterBattery(previous_battery,
                                     PowerAdcDriver_RawToVoltage(raw_battery) * POWER_ADC_DRIVER_BATTERY_DIVIDER);
    ...
    next_state.raw_sample_count          = PowerAdcDriver_ClampU16(raw_sample_count);
    ...
    if (samples_ready != 0U)
    {
        next_state.valid_flags |= POWER_ADC_DRIVER_VALID_SAMPLES_READY;
    }
    if (zero_valid != 0U)
    {
        next_state.valid_flags |= POWER_ADC_DRIVER_VALID_CURRENT_ZERO_READY;
    }
    if (samples_ready == 0U)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_NOT_READY;
    }
    if (zero_valid == 0U)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_ZERO_CALIBRATING;
    }
    if (raw_sample_count == 0UL)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_NO_NEW_SAMPLE;
        if (zero_valid != 0U)
        {
            next_state.missed_window_count = PowerAdcDriver_ClampU16((uint32_t)next_state.missed_window_count + 1UL);
        }
    }
    if (dma_error_latched != 0U)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_DMA_ERROR;
    }
    if (zero_valid != 0U && delta_sample_count == 0UL)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_SAMPLE_RATE;
    }
    if (zero_valid != 0U && delta_sample_count > 0UL
        && delta_sample_count < POWER_ADC_DRIVER_CONTROL_MIN_WINDOW_SAMPLES)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_WINDOW_TOO_SMALL;
    }
    if (zero_valid != 0U && delta_sample_count > POWER_ADC_DRIVER_CONTROL_MAX_WINDOW_SAMPLES)
    {
        next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_SAMPLE_RATE;
    }
    next_current_valid =
        (samples_ready != 0U && zero_valid != 0U && raw_sample_count != 0UL && delta_sample_count != 0UL
         && dma_error_latched == 0U && POWER_ADC_DRIVER_CALIBRATION_ENABLED != 0U && MOTOR_CURRENT_VOLTS_PER_AMP > 0.0f)
            ? 1U
            : 0U;
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        if (zero_span_raw[i] > POWER_ADC_DRIVER_CURRENT_ZERO_MAX_SPAN_RAW)
        {
            quality_flags |= POWER_ADC_DRIVER_QUALITY_ZERO_UNSTABLE;
            next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_ZERO_UNSTABLE;
        }
        if (delta_sample_count > 0UL && delta_sample_count < POWER_ADC_DRIVER_CONTROL_MIN_WINDOW_SAMPLES)
        {
            quality_flags |= POWER_ADC_DRIVER_QUALITY_WINDOW_TOO_SMALL;
        }
        if (delta_max[i] > POWER_ADC_DRIVER_CURRENT_SPIKE_MAX_RAW)
        {
            quality_flags |= POWER_ADC_DRIVER_QUALITY_WINDOW_SPIKE;
            next_state.invalid_reason_flags |= POWER_ADC_DRIVER_INVALID_WINDOW_SPIKE;
        }
        if (next_current_valid != 0U && delta_sample_count != 0UL)
        {
            PowerAdcDriver_ComputeCurrentWindow(delta_sum[i],
                                                delta_sq_sum[i],
                                                signed_delta_sum[i],
                                                ...
                                                &noise_current,
                                                &trimmed_current);
            next_state.current_a[i]             = PowerAdcDriver_FilterCurrent(previous_current[i], trimmed_current);
            ...
            next_state.current_sample_count[i]  = PowerAdcDriver_ClampU16(delta_sample_count);
        }
        ...
        else if (MotorHardwareLayout_MotorSide((motor_id_t)i) == MOTOR_SIDE_LEFT)
        {
            left_count++;
        ...
        if (next_current_valid != 0U && quality_flags == 0UL && MotorHardwareLayout_MotorEnabled((motor_id_t)i) != 0U)
        {
            next_state.current_control_valid_mask |= (uint8_t)(1U << i);
        }
    }
    if (delta_sample_count < POWER_ADC_DRIVER_CONTROL_MIN_WINDOW_SAMPLES
        || delta_sample_count > POWER_ADC_DRIVER_CONTROL_MAX_WINDOW_SAMPLES)
    {
        next_current_control_valid            = 0U;
...
void PowerAdcDriver_ApplyCurrentZeroCalibration(const uint16_t zero_raw[MOTOR_ID_COUNT])
{
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        current_zero_span_raw[i]           = 0U;
        current_zero_sum[i]                = (uint32_t)zero_raw[i] * POWER_ADC_DRIVER_CURRENT_ZERO_SAMPLES;
        adc_state.current_zero_raw[i]      = zero_raw[i];
        adc_state.current_zero_span_raw[i] = 0U;
    }
    adc_state.current_zero_valid        = 1U;
    adc_state.current_zero_sample_count = POWER_ADC_DRIVER_CURRENT_ZERO_SAMPLES;
...
void PowerAdcDriver_SetCurrentZeroStationary(uint8_t stationary)
{
    ...
    {
        PowerAdcDriver_ResetCurrentZeroAccumulators();
        PowerAdcDriver_ResetWindowAccumulators();
        adc_state.current_zero_sample_count = 0U;
        adc_state.current_zero_valid        = 0U;
        for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
