    ...
    if (total_count >= ADC_MONITOR_CURRENT_ZERO_SAMPLES)
    {
      for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
      {
        ...
        current_zero_span_raw[i] = (current_zero_max_raw[i] >= current_zero_min_raw[i]) ?
                                   (uint16_t)(current_zero_max_raw[i] - current_zero_min_raw[i]) : 0U;
      }
...
void AdcMonitor_Update(void)
{
  ...
  if (zero_valid != 0U && delta_sample_count > 0UL &&
      delta_sample_count < ADC_MONITOR_CONTROL_MIN_WINDOW_SAMPLES)
  {
  ...
  for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
  {
    ...
    if (next_current_valid != 0U &&
        quality_flags == 0UL &&
        ChassisLayout_MotorEnabled((motor_id_t)i) != 0U)
    {
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
  ...
  for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
  {
    ...
    if (zero_valid != 0U)
    {
      ...
      uint32_t signed_abs_delta = (signed_delta < 0) ?
