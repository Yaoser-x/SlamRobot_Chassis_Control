...
static const imu_bmi270_profile_t profiles[] = {
  {
...
uint8_t ImuBmi270Profile_Check(const imu_bmi270_profile_t *profile,
                               const imu_bmi270_profile_check_t *check)
{
  ...
  return (profile->acc_conf == check->acc_conf &&
          profile->acc_range == check->acc_range &&
          profile->gyr_conf == check->gyr_conf &&
          profile->gyr_range == check->gyr_range &&
          profile->pwr_conf == check->pwr_conf &&
          profile->pwr_ctrl == check->pwr_ctrl &&
          profile->fifo_config_0 == check->fifo_config_0 &&
          profile->fifo_config_1 == check->fifo_config_1 &&
          profile->int1_io_ctrl == check->int1_io_ctrl &&
          profile->int_map_data == check->int_map_data) ? 1U : 0U;
}
