...
static bmi270_driver_state_t            imu_state;
...
static bmi270_calibration_t             imu_calibration;
static imu_bmi270_mahony_t              bmi270_attitude_filter;
static imu_bmi270_mahony_params_t       bmi270_attitude_filter_params;
static volatile uint8_t                 imu_gyro_calibration_active;
static bmi270_calibration_accumulator_t imu_gyro_cal_accumulator;
static uint8_t                          imu_gyro_calibration_is_auto;
static uint32_t                         imu_gyro_cal_last_imu_sample_count;

static void Bmi270Driver_ServiceAutoCal(uint32_t now_ms, uint8_t stationary);
...
static float Bmi270Driver_AbsFloat(float value)
{
    return (value < 0.0f) ? -value : value;
}

static void Bmi270Driver_UpdateGyroCalDiag(uint8_t     reason,
                                           uint8_t     axis,
...
static void Bmi270Driver_ScheduleAutoCal(uint32_t now_ms, uint32_t delay_ms)
{
...
static void Bmi270Driver_SetError(uint8_t error)
{
    uint32_t primask = __get_PRIMASK();
...
static void Bmi270Driver_SetQuality(uint32_t flags)
{
    uint32_t primask = __get_PRIMASK();
...
static void Bmi270Driver_ClearTransientQuality(void)
{
...
uint8_t Bmi270Driver_ReadReg(uint8_t reg, uint8_t *value)
{
    if (Bmi270Bus_ReadReg(reg, value) == 0U)
    {
        Bmi270Driver_SetError(IMU_BMI270_ERROR_SPI);
        return 0U;
    }
    return 1U;
}
...
static uint8_t Bmi270Driver_ReadBytes(uint8_t reg, uint8_t *data, uint8_t len)
{
...
static uint8_t Bmi270Driver_ReadRawFrame(int16_t accel_raw[3], int16_t gyro_raw[3])
{
    uint8_t data[12];
    ...
    {
        Bmi270Driver_SetError(IMU_BMI270_ERROR_READ);
        return 0U;
    }

    accel_raw[0] = Bmi270Driver_ReadI16(&data[0]);
    accel_raw[1] = Bmi270Driver_ReadI16(&data[2]);
    accel_raw[2] = Bmi270Driver_ReadI16(&data[4]);
    gyro_raw[0]  = Bmi270Driver_ReadI16(&data[6]);
    gyro_raw[1]  = Bmi270Driver_ReadI16(&data[8]);
    gyro_raw[2]  = Bmi270Driver_ReadI16(&data[10]);
    if (Bmi270Driver_RawFrameHasSignal(accel_raw, gyro_raw) == 0U)
    {
        Bmi270Driver_SetError(IMU_BMI270_ERROR_INVALID_FRAME);
        return 0U;
    }
    return 1U;
}
...
static void Bmi270Driver_UpdateTemperature(void)
{
    ...
    if (Bmi270Driver_ReadBytes(BMI270_REG_TEMP_0, data, sizeof(data)) == 0U
        || Bmi270Driver_TemperatureRawToC(Bmi270Driver_ReadI16(data), &temperature_c) == 0U)
    {
        imu_state.temperature_valid = 0U;
        Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_TEMPERATURE_INVALID);
        return;
    }
...
static uint8_t Bmi270Driver_ReadSensorTime(uint32_t *sensor_time)
{
    uint8_t data[3];
    ...
    if (Bmi270Driver_ReadBytes(BMI270_REG_SENSORTIME_0, data, sizeof(data)) == 0U)
    {
        return 0U;
    }
...
static uint8_t Bmi270Driver_ReadFifoLength(uint16_t *fifo_len)
{
    uint8_t data[2];
...
uint8_t Bmi270Driver_WriteReg(uint8_t reg, uint8_t value)
{
...
static uint8_t Bmi270Driver_WriteBytes(uint8_t reg, const uint8_t *data, uint8_t len)
{
...
static uint8_t Bmi270Driver_HandleDeviceStatus(bmi270_device_status_t status)
{
    if (status == BMI270_DEVICE_CONFIG_ERROR)
    {
        Bmi270Driver_SetError(IMU_BMI270_ERROR_CONFIG);
    }
    else if (status == BMI270_DEVICE_PROFILE_MISMATCH)
    {
        Bmi270Driver_SetError(IMU_BMI270_ERROR_PROFILE_VERIFY);
    }
    return (status == BMI270_DEVICE_OK) ? 1U : 0U;
}

static uint8_t Bmi270Driver_LoadConfigFile(void)
{
    return Bmi270Driver_HandleDeviceStatus(Bmi270Device_LoadConfig(&imu_device_io));
}

static uint8_t Bmi270Driver_WaitInitOk(void)
{
    return Bmi270Driver_HandleDeviceStatus(Bmi270Device_WaitInitOk(&imu_device_io));
}

static uint8_t Bmi270Driver_ApplyProfile(const imu_bmi270_profile_t *profile)
{
    return Bmi270Driver_HandleDeviceStatus(Bmi270Device_ApplyProfile(&imu_device_io, profile));
}

void Bmi270Driver_Init(void)
{
    imu_state                       = (bmi270_driver_state_t){0};
    ...
    Bmi270CalibrationAccumulator_Init(&imu_gyro_cal_accumulator);
    Bmi270Calibration_Default(&imu_calibration);
    bmi270_attitude_filter_params = ImuBmi270Mahony_DefaultParams();
    ImuBmi270Mahony_Init(&bmi270_attitude_filter);
    Bmi270Bus_Deselect();
}
...
uint8_t Bmi270Driver_SetEnabled(uint8_t enabled)
{
    ...
    {
        Bmi270Driver_ScheduleAutoCal(HAL_GetTick(), BMI270_GYRO_AUTO_CAL_START_DELAY_MS);
    }
    return 1U;
}
...
uint8_t Bmi270Driver_ProbeNow(void)
{
    ...
    {
        Bmi270Driver_SetError(IMU_BMI270_ERROR_CHIP_ID);
        return 0U;
    }

    Bmi270Driver_SetError(IMU_BMI270_ERROR_NONE);
    return 1U;
}
...
uint8_t Bmi270Driver_ConfigNow(void)
{
    ...
    if (Bmi270Driver_WriteReg(BMI270_REG_CMD, BMI270_CMD_SOFT_RESET) == 0U)
    {
        return 0U;
    }
    HAL_Delay(5U);
    if (Bmi270Driver_ProbeNow() == 0U)
    {
        return 0U;
    }
    imu_state.online = 0U;

    if (Bmi270Driver_WriteReg(BMI270_REG_PWR_CONF, profile->pwr_conf) == 0U)
    {
        return 0U;
    }
    HAL_Delay(BMI270_SPI_SELECT_DELAY_MS);
    ...
    if (Bmi270Driver_LoadConfigFile() == 0U)
    {
        return 0U;
    }
    if (Bmi270Driver_WaitInitOk() == 0U)
    {
        return 0U;
    }
    imu_state.init_state = IMU_BMI270_INIT_STATE_VERIFY_PROFILE;
    if (Bmi270Driver_ApplyProfile(profile) == 0U)
    {
        return 0U;
    }
    imu_state.enabled            = 1U;
...
static void Bmi270Driver_ProcessMeasurement(const int16_t accel_raw[3],
                                            ...
                                            float         fallback_dt_s)
{
    ...
    primask = __get_PRIMASK();
    __disable_irq();
    Bmi270Driver_ClearTransientQuality();
    __set_PRIMASK(primask);
    ...
    for (uint8_t i = 0U; i < 3U; ++i)
    {
        ...
        gyro_corrected_dps[i] = gyro_raw_dps[i]
                                - Bmi270Calibration_GyroBiasAtTemperature(&imu_calibration,
                                                                          i,
                                                                          imu_state.temperature_c,
                                                                          imu_state.temperature_valid);
        if (Bmi270Driver_AbsFloat(gyro_raw_dps[i]) >= BMI270_GYRO_SATURATION_DPS)
        {
            Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_GYRO_SATURATION);
        }
    }
    ...
    {
        Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_ACCEL_ANOMALY);
    }
    ...
    if (sensor_time_valid != 0U && imu_state.sensor_time_valid != 0U)
    {
        ...
        {
            Bmi270Driver_SetError(IMU_BMI270_ERROR_TIMESTAMP);
            Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_ATTITUDE_INVALID);
        }
    }
    else if (fallback_dt_s > 0.0f)
    {
        dt_valid = 1U;
        Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_POLL_FALLBACK);
    }

    if (dt_valid != 0U)
    {
        ImuBmi270Mahony_Update(&bmi270_attitude_filter,
                               body_gyro_dps,
                               body_accel_g,
                               dt_s,
                               &bmi270_attitude_filter_params);
        if ((bmi270_attitude_filter.status_flags & IMU_BMI270_FUSION_ACCEL_DEGRADED) != 0UL)
        {
            Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_ACCEL_ANOMALY);
        }
        if ((bmi270_attitude_filter.status_flags & IMU_BMI270_FUSION_INVALID_DT) != 0UL)
        {
            Bmi270Driver_SetQuality(IMU_BMI270_QUALITY_ATTITUDE_INVALID);
        }
    }
    ImuBmi270Quaternion_ToEulerDeg(&bmi270_attitude_filter.q, euler_deg);
    ...
    imu_state.quaternion[0]           = bmi270_attitude_filter.q.w;
    imu_state.quaternion[1]           = bmi270_attitude_filter.q.x;
    imu_state.quaternion[2]           = bmi270_attitude_filter.q.y;
    imu_state.quaternion[3]           = bmi270_attitude_filter.q.z;
    imu_state.roll_deg                = euler_deg[0];
    imu_state.pitch_deg               = euler_deg[1];
    imu_state.yaw_deg                 = ImuSignalFilter_WrapAngleDeg(euler_deg[2]);
    imu_state.accel_correction_weight = bmi270_attitude_filter.accel_weight;
...
static uint8_t Bmi270Driver_UpdateDirect(float fallback_dt_s)
{
    int16_t  accel_raw[3];
    int16_t  gyro_raw[3];
    ...
    if (Bmi270Driver_ReadRawFrame(accel_raw, gyro_raw) == 0U)
    {
        return 0U;
    }
    sensor_time_valid = Bmi270Driver_ReadSensorTime(&sensor_time);
    Bmi270Driver_ProcessMeasurement(accel_raw, gyro_raw, sensor_time, sensor_time_valid, fallback_dt_s);
    return 1U;
}
...
static uint8_t Bmi270Driver_UpdateFifo(void)
{
    ...
    if (Bmi270Driver_ReadFifoLength(&fifo_len) == 0U)
    {
        return 0U;
    }
    if (fifo_len == 0U)
    ...
    if (fifo_len > BMI270_FIFO_READ_MAX_BYTES)
    {
        ...
        Bmi270Driver_SetError(IMU_BMI270_ERROR_FIFO);
    }
    if (Bmi270Driver_ReadBytes(BMI270_REG_FIFO_DATA, fifo, (uint8_t)fifo_len) == 0U)
    {
        return 0U;
    }
    ...
    for (uint32_t i = 0UL; i < parse.sample_count && i < BMI270_FIFO_MAX_SAMPLES; ++i)
    {
        ...
        {
            if (Bmi270Driver_RawFrameHasSignal(samples[i].accel_raw, samples[i].gyro_raw) != 0U)
            {
                Bmi270Driver_ProcessMeasurement(samples[i].accel_raw,
                                                samples[i].gyro_raw,
                                                sample_time,
                                                sample_time_valid,
                                                0.0f);
...
uint8_t Bmi270Driver_Update(void)
{
    ...
    if (Bmi270Driver_UpdateDirect(BMI270_DIRECT_FALLBACK_DT_S) == 0U)
    {
        return 0U;
    }
...
static uint8_t Bmi270Driver_BeginGyroCalibrationRequest(uint16_t samples, uint16_t delay_ms, uint8_t is_auto)
{
    ...
    if (Bmi270CalibrationAccumulator_Begin(&imu_gyro_cal_accumulator, samples, delay_ms) == 0U)
    {
        return 0U;
    }
    ...
    imu_state.gyro_auto_cal_state       = IMU_BMI270_GYRO_AUTO_CAL_WAIT;
    Bmi270Driver_UpdateGyroCalDiag(IMU_BMI270_GYRO_CAL_FAIL_NONE, 0xFFU, 0, 0, 0, 0U);
    return 1U;
}

static void
Bmi270Driver_FinishGyroCalibrationFailure(uint32_t now_ms, uint8_t reason, uint8_t axis, uint8_t consume_attempt)
{
    Bmi270Driver_UpdateGyroCalDiag(reason,
                                   axis,
    ...
    if (imu_gyro_calibration_is_auto != 0U)
    {
        ...
        {
            Bmi270Driver_ScheduleAutoCal(now_ms, BMI270_GYRO_AUTO_CAL_RETRY_MS);
...
static void Bmi270Driver_ServiceAutoCal(uint32_t now_ms, uint8_t stationary)
{
    ...
    if (Bmi270Driver_AutoCalDue(imu_state.gyro_auto_cal_enabled,
                                imu_state.online,
    ...
    if (Bmi270Driver_BeginGyroCalibrationRequest(BMI270_GYRO_AUTO_CAL_SAMPLES, BMI270_GYRO_AUTO_CAL_DELAY_MS, 1U) != 0U)
    {
...
{
    return Bmi270Driver_BeginGyroCalibrationRequest(samples, delay_ms, 0U);
}
...
void Bmi270Driver_ServiceCalibration(uint32_t now_ms, uint8_t stationary)
{
    ...
    bmi270_calibration_acc_state_t result;

    Bmi270Driver_ServiceAutoCal(now_ms, stationary);
    if (imu_gyro_calibration_active == 0U)
    {
        return;
    }
    if (stationary == 0U)
    {
        Bmi270Driver_FinishGyroCalibrationFailure(now_ms, IMU_BMI270_GYRO_CAL_FAIL_MOTION, 0xFFU, 0U);
        return;
    }
    ...
    result = Bmi270CalibrationAccumulator_Feed(&imu_gyro_cal_accumulator,
                                               now_ms,
                                               accel_g,
                                               gyro_dps,
                                               BMI270_GYRO_CAL_MAX_ABS_DPS,
                                               BMI270_GYRO_CAL_STILL_SPAN_DPS);
    Bmi270Driver_UpdateGyroCalDiag(IMU_BMI270_GYRO_CAL_FAIL_NONE,
                                   0xFFU,
                                   imu_gyro_cal_accumulator.sum_dps,
                                   imu_gyro_cal_accumulator.min_dps,
                                   imu_gyro_cal_accumulator.max_dps,
                                   imu_gyro_cal_accumulator.sample_count);
    if (result == BMI270_CALIBRATION_ACC_COLLECTING)
    {
        imu_state.gyro_auto_cal_state = IMU_BMI270_GYRO_AUTO_CAL_RUNNING;
        return;
    }
    if (result == BMI270_CALIBRATION_ACC_FAIL_ABS)
    {
        Bmi270Driver_FinishGyroCalibrationFailure(now_ms,
                                                  IMU_BMI270_GYRO_CAL_FAIL_ABS,
                                                  imu_gyro_cal_accumulator.fail_axis,
                                                  1U);
        return;
    }
    if (result == BMI270_CALIBRATION_ACC_FAIL_SPAN)
    {
        Bmi270Driver_FinishGyroCalibrationFailure(now_ms,
    ...
    if (result == BMI270_CALIBRATION_ACC_READY
        && Bmi270CalibrationAccumulator_GetResult(&imu_gyro_cal_accumulator, bias_dps, accel_mean_g) != 0U)
    {
        ...
        {
            bmi270_attitude_filter.q            = calibrated_q;
            bmi270_attitude_filter.integral[0]  = 0.0f;
            bmi270_attitude_filter.integral[1]  = 0.0f;
            bmi270_attitude_filter.integral[2]  = 0.0f;
            bmi270_attitude_filter.accel_weight = 1.0f;
            bmi270_attitude_filter.status_flags = 0UL;
            bmi270_attitude_filter.initialized  = 1U;
        }
        else
        {
            ImuBmi270Mahony_Init(&bmi270_attitude_filter);
...
uint8_t Bmi270Driver_ApplyCalibration(const bmi270_calibration_t *calibration)
{
    ...
    if (Bmi270Calibration_Validate(calibration) == 0U)
    {
        return 0U;
    }
    primask = __get_PRIMASK();
