    ...
    uint8_t temperature_valid)
{
  ...
  {
    bias += calibration->temperature_gyro_slope_dps_per_c[axis] *
            (temperature_c - calibration->temperature_offset_c);
  }
  return bias;
}
...
imu_bmi270_gyro_cal_acc_state_t ImuBmi270GyroCalAccumulator_Feed(
    ...
    float max_span_dps)
{
  if (accumulator == 0 || accel_g == 0 || gyro_dps == 0 ||
      accumulator->state != IMU_BMI270_GYRO_CAL_ACC_COLLECTING)
  {
    return (accumulator == 0) ? IMU_BMI270_GYRO_CAL_ACC_IDLE :
                               (imu_bmi270_gyro_cal_acc_state_t)accumulator->state;
  }
  if (accumulator->has_last_sample != 0U &&
      (now_ms - accumulator->last_sample_ms) < accumulator->interval_ms)
  {
...
{
  if (accumulator == 0 || bias_dps == 0 || accel_mean_g == 0 ||
      accumulator->state != IMU_BMI270_GYRO_CAL_ACC_READY ||
