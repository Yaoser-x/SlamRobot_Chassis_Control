...
uint8_t ParamStore_Validate(const param_store_t *params)
{
    ...
    for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
    {
        ...
        if (ParamStore_Finite(params->pid_kp[i]) == 0U || ParamStore_Finite(params->pid_ki[i]) == 0U
...
static uint8_t ParamStore_FindFloat(param_store_t *params, const char *name, param_float_binding_t *binding)
{
...
static uint8_t ParamStore_FindInt(param_store_t *params, const char *name, param_int_binding_t *binding)
{
