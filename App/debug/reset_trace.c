...
void ResetTrace_CaptureWithTask(reset_trace_kind_t kind, uint32_t reason, uint32_t line, reset_trace_task_t task)
{
    uint32_t primask = ResetTrace_EnterCritical();
...
void ResetTrace_CaptureFaultStack(reset_trace_kind_t kind, const uint32_t *stack, uint32_t exc_return)
{
    uint32_t           primask     = ResetTrace_EnterCritical();
    reset_trace_task_t task        = RESET_TRACE_TASK_NONE;
