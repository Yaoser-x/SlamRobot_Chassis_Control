...
static void DebugConsole_Write(const char *text)
{
    if (text != 0)
    {
        (void)HAL_UART_Transmit(&huart1, (uint8_t *)text, (uint16_t)strlen(text), DEBUG_CONSOLE_TX_TIMEOUT_MS);
    }
}
...
static void
DebugConsole_GetMotorLogSpeed(uint32_t now_ms, const encoder_state_t *state, float speed_mps[MOTOR_ID_COUNT])
{
...
static void DebugConsole_PrintFilteredLogFrame(uint32_t now_ms)
{
    ...
    {
        tx[pos++] = ',';
        pos       = DebugConsole_WriteFieldData(tx, pos, (log_field_id_t)log_filter_order[i], now_ms);
    }
    pos += (size_t)snprintf(tx + pos, sizeof(tx) - pos, "\r\n");
    DebugConsole_Write(tx);
}
...
static void DebugConsole_PrintLineStatus(void)
{
    ...
    (void)snprintf(
        ...
        lc_state.globally_enabled,
        lc_state.tracking_active,
        (double)lc_state.line_position,
        (double)lc_state.error,
        (double)lc_state.error_derivative,
        (double)lc_state.linear_x,
        (double)lc_state.angular_z,
        lc_state.detected_count,
        lc_state.output_saturated,
        lc_state.lost_reason,
        lc_state.active_low,
        sensor.state[0],
        sensor.state[1],
        sensor.state[2],
...
static void DebugConsole_HandleLine(char *line)
{
    ...
    else if ((strcmp(line, "imucal") == 0) || (sscanf(line, "imucal %d", &value) == 1))
    {
        ...
        {
            ImuBmi270_GetState(&imu_state);
            LOG_WARN("bmi270 gyro calibration failed reason=%s axis=%u samples=%u mean_dps=%.2f,%.2f,%.2f "
                     "span_dps=%.2f,%.2f,%.2f min_dps=%.2f,%.2f,%.2f max_dps=%.2f,%.2f,%.2f init=%u err=%u chip=0x%02X "
                     "online=%u",
            ...
            else if (strcmp(action, "show") == 0)
            {
                ...
                for (uint8_t channel = 0U; channel < LINE_CALIBRATION_CHANNELS; ++channel)
                {
                    uint16_t floor_mean  = (calibration.count[0] != 0U)
                                               ? (uint16_t)(calibration.sum[0][channel] / calibration.count[0])
                                               : 0U;
                    uint16_t line_mean   = (calibration.count[1] != 0U)
                                               ? (uint16_t)(calibration.sum[1][channel] / calibration.count[1])
                                               : 0U;
                    uint16_t floor_range = (calibration.count[0] != 0U)
                                               ? (uint16_t)(calibration.max[0][channel] - calibration.min[0][channel])
                                               : 0U;
                    uint16_t line_range  = (calibration.count[1] != 0U)
                                               ? (uint16_t)(calibration.max[1][channel] - calibration.min[1][channel])
                                               : 0U;
    ...
    else if (strcmp(line, "i2cscan") == 0)
    {
        ...
        {
            HAL_StatusTypeDef st = HAL_I2C_IsDeviceReady(&hi2c1, (uint16_t)(addr << 1), 1, 5);
            if (st == HAL_OK)
            {
                found = 1U;
                pos += snprintf(tx + pos, sizeof(tx) - pos, "  0x%02X (7-bit)  ACK\r\n", addr);
            }
        }
        if (found == 0U)
        {
