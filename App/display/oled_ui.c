...
static const char *const sc_names[SC_COUNT] = {"I2C ", "IMU ", "ADC ", "Motr", "Enc ", "RPI ", "Line", "ESP "};
 ...
 * ================================================================ */
static void OLED_UI_DrawChineseStr(uint8_t        x,
                                   uint8_t        page,
                                   const char    *str,
...
static uint8_t OLED_UI_RunSelfCheck(selfcheck_item_t item)
{
    switch (item)
    {
        case SC_I2C:
        {
            HAL_StatusTypeDef status = HAL_I2C_IsDeviceReady(&hi2c1, (uint16_t)(OLED_I2C_ADDR << 1), 2, 10);
        ...
        case SC_UART4_LINE:
        {
            ...
            return (uint8_t)OLED_SelfCheckLine(now, valid, sensor.timestamp_ms, OLED_MODULE_TIMEOUT_LINE_MS);
        }
...
static void OLED_UI_DrawWelcome(void)
{
    ...
    /* Subtitle: "四轮差速底盘控制" 16x16 Chinese, pages 2-3 (y=16~31) */
    {
        ...
        OLED_UI_DrawChineseStr(cx, 2, line1, OLED_FONT_16X16_DATA, OLED_FONT_16X16_CHARS, 16, 16);
    }
    ...
    /* "系统启动中..." (pages 6-7, y=48~63, centered) */
    {
        ...
        OLED_UI_DrawChineseStr(cx, 6, line3, OLED_FONT_16X16_DATA, OLED_FONT_16X16_CHARS, 16, 16);
    }
}
...
static void OLED_UI_DrawSelfCheck(void)
{
    /* Title: "系统自检中" 16x16 Chinese, pages 0-1 (y=0~15) */
    {
        ...
        uint8_t     cx    = (uint8_t)((128 - 5 * 16) / 2);
        OLED_UI_DrawChineseStr(cx, 0, title, OLED_FONT_16X16_DATA, OLED_FONT_16X16_CHARS, 16, 16);
    }
...
static void OLED_UI_DrawNormal(void)
{
    ...
    if (imu.gyro_auto_cal_state != last_calibration_state)
    {
        ...
        calibration_terminal_since_ms = (imu.gyro_auto_cal_state == IMU_BMI270_GYRO_AUTO_CAL_DONE
                                         || imu.gyro_auto_cal_state == IMU_BMI270_GYRO_AUTO_CAL_FAILED)
                                            ? now
                                            : 0U;
    }
    ...
    /* Row 0: Runtime (pages 0~1) */
    {
        ...
        OLED_UI_DrawChineseStr(64, 0, "运行时间", OLED_FONT_16X16_DATA, OLED_FONT_16X16_CHARS, 16, 16);
    }
    ...
    /* Row 1: Battery voltage (pages 2~3) */
    {
        ...
        OLED_UI_DrawChineseStr(88, 2, "电池", OLED_FONT_16X16_DATA, OLED_FONT_16X16_CHARS, 16, 16);
    }
