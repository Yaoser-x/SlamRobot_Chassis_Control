...
void LineControl_Update(void)
{
  ...
  {
    derivative = (error - g_previous_error) * 1000.0f /
      (float)(uint32_t)(sensor.timestamp_ms - g_previous_timestamp_ms);
  }
  angular_z = params.line_kp * error + params.line_kd * derivative;
  g_line_state.output_saturated =
    (angular_z > LINE_ANGULAR_MAX_RPS || angular_z < -LINE_ANGULAR_MAX_RPS) ? 1U : 0U;
  angular_z = LineControl_ClampFloat(angular_z, LINE_ANGULAR_MAX_RPS);
  speed = params.line_speed_mps / (1.0f + params.line_slowdown_gain *
...
uint8_t LineControl_CalibrationBuild(uint16_t thresholds[LINE_CALIBRATION_CHANNELS],
                                     uint8_t *active_low)
{
