...
void LineCalibration_Init(line_calibration_t *calibration)
{
  if (calibration != 0)
  {
...
uint8_t LineCalibration_Begin(line_calibration_t *calibration,
...
void LineCalibration_Feed(line_calibration_t *calibration,
                          const uint16_t raw[LINE_CALIBRATION_CHANNELS])
{
  uint8_t index;
...
uint8_t LineCalibration_Apply(line_calibration_t *calibration,
                              ...
                              uint8_t *active_low)
{
  ...
  if (calibration == 0 || thresholds == 0 || active_low == 0 ||
  ...
  for (uint8_t channel = 0U; channel < LINE_CALIBRATION_CHANNELS; ++channel)
  {
    ...
    uint16_t separation = (floor_mean > line_mean) ?
                            (uint16_t)(floor_mean - line_mean) :
                            (uint16_t)(line_mean - floor_mean);
