...
static void ChassisTasks_ServiceFirstImuCalibrationSave(uint32_t now_ms)
{
    ...
    if (imu_calibration_save_pending == 0U || ((int32_t)(now_ms - imu_calibration_save_next_ms)) < 0)
    {
        return;
    }
...
void ChassisTasks_InitHardware(void)
{
    ...
    if (params_loaded != 0U)
    {
        ...
        imu_first_calibration_save_needed =
...
void Task_Imu(void *argument)
{
    ...
    for (;;)
    {
        ...
        (void)osThreadFlagsWait(CHASSIS_IMU_TASK_FLAG_DRDY, osFlagsWaitAny, CHASSIS_IMU_PERIOD_MS);
        ...
        ImuBmi270_GetState(&imu_state);
        if (imu_first_calibration_save_needed != 0U && was_calibrated == 0U && imu_state.gyro_calibrated != 0U)
        {
