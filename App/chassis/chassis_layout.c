...
{
  if (ChassisLayout_IsValidMotor(motor) == 0U ||
      motor_layout[(uint32_t)motor].motor_direction < 0)
  {
    return -1;
  }
  return 1;
...
{
  if (ChassisLayout_IsValidMotor(motor) == 0U ||
      motor_layout[(uint32_t)motor].encoder_direction < 0)
  {
    return -1;
  }
  return 1;
...
uint8_t ChassisLayout_SideMotorCount(motor_side_t side)
{
  ...
  for (uint8_t i = 0U; i < MOTOR_ID_COUNT; ++i)
  {
    if (ChassisLayout_MotorEnabled((motor_id_t)i) != 0U &&
        ChassisLayout_MotorSide((motor_id_t)i) == side)
    {
      count++;
    }
  }
...
{
  return (ChassisLayout_SideMotorCount(MOTOR_SIDE_LEFT) != 0U &&
          ChassisLayout_SideMotorCount(MOTOR_SIDE_RIGHT) != 0U) ? 1U : 0U;
}
