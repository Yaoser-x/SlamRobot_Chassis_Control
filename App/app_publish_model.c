...
static app_publish_model_config_t    publish_config;

static uint8_t AppPublishModel_IsFresh(uint32_t now_ms, uint32_t timestamp_ms, uint32_t timeout_ms)
{
...
void AppPublishModel_Update(uint32_t now_ms)
{
    ...
    next.modules.upper_online =
        AppPublishModel_IsFresh(now_ms, next.communication.upper_last_rx_timestamp_ms, publish_config.host_timeout_ms);
    next.modules.esp12f_online =
        (esp.boot_mode_download == 0U)
            ? AppPublishModel_IsFresh(now_ms, esp.last_rx_timestamp_ms, publish_config.esp12f_timeout_ms)
            : 0U;
    next.modules.line_online =
        (line.sensor_valid != 0U)
            ? AppPublishModel_IsFresh(now_ms, line.sensor_timestamp_ms, publish_config.line_timeout_ms)
            : 0U;
