...
static void ImuCalibrationGate_ResetEvidence(imu_calibration_gate_t *gate,
...
static uint8_t ImuCalibrationGate_EvidenceStable(const imu_calibration_gate_t *gate)
{
  float sample_count;
  float mean;
  float variance;
    ...
    uint32_t imu_sample_count)
{
  ...
  if (gate == 0 || effective_pwm == 0 || speed_mps == 0 || speed_valid == 0 ||
      accel_g == 0 || gyro_dps == 0 || motor_enabled_mask == 0U)
  {
    return 0U;
  }
  for (uint8_t motor = 0U; motor < IMU_CALIBRATION_GATE_MOTOR_COUNT; ++motor)
  {
    ...
    {
      ImuCalibrationGate_ResetEvidence(gate, imu_sample_count);
      return 0U;
    }
    if ((motor_enabled_mask & (uint8_t)(1U << motor)) != 0U &&
        (speed_valid[motor] == 0U ||
  ...
  accel_norm = sqrtf((accel_g[0] * accel_g[0]) +
                     (accel_g[1] * accel_g[1]) +
                     (accel_g[2] * accel_g[2]));
  if (accel_norm < IMU_CALIBRATION_GATE_ACCEL_MIN_G ||
      accel_norm > IMU_CALIBRATION_GATE_ACCEL_MAX_G)
  {
    ImuCalibrationGate_ResetEvidence(gate, imu_sample_count);
    return 0U;
  }
  ...
  {
    return ImuCalibrationGate_EvidenceStable(gate);
  }
  if (gate->sample_count >= IMU_CALIBRATION_GATE_WINDOW_SAMPLES)
  {
