...
static void UpperUart_TryStartTxLocked(void)
{
    ...
    status = HAL_UART_Transmit_IT(&huart3, upper_tx_queue[selected].data, upper_tx_queue[selected].length);
    if (status == HAL_OK)
    {
...
static uint8_t UpperUart_EnqueueTx(const uint8_t *frame, uint16_t frame_len, upper_tx_priority_t priority)
{
    uint32_t primask;
    uint8_t  selected = UPPER_UART_TX_SLOT_INVALID;
...
static void UpperUart_SendDiagnostic(uint32_t now_ms)
{
    ...
    if (frame_len > 0U && UpperUart_EnqueueTx(upper_diagnostic_tx_frame, frame_len, UPPER_TX_PRIORITY_DIAGNOSTIC) != 0U)
    {
