...
static void Esp12fComm_SendDiagnostic(uint32_t now_ms)
{
    ...
    if ((now_ms - esp12f_last_diagnostic_ms) < UPPER_DIAGNOSTIC_PERIOD_MS || huart2.gState != HAL_UART_STATE_READY)
    {
        return;
    }
    ...
    if (frame_len > 0U && HAL_UART_Transmit_IT(&huart2, esp12f_diagnostic_tx_frame, frame_len) == HAL_OK)
    {
