...
默认两驱配置仅启用 M1（左侧）和 M3（右侧），M2/M4 的速度、电流和故障状态会按禁用策略归零。V2.0 实板 M2/M3 的最终映射为：

| 逻辑电机 | PWM IN1/IN2 | nFAULT | 编码器 | IPROPI |
| --- | --- | --- | --- | --- |
...
依次测试需要启用的电机；默认两驱重点测试 M1、M3。**确认项**：
- 每路电机正转/反转方向与预期一致
...
**确认项**：`CHASSIS_Mx_SIDE` 侧别配置与实际轮组对应，聚合命令下所有已启用目标电机同步动作。默认两驱下 `left 200` 仅驱动 M1，`right 200` 仅驱动 M3。
