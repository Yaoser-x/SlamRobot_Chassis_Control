...
static float RelativeHeadingController_Abs(float value)
{
    return (value < 0.0f) ? -value : value;
}

void RelativeHeadingController_Init(relative_heading_controller_t *control)
{
    if (control != 0)
    {
        *control = (relative_heading_controller_t){0};
    }
}

uint8_t RelativeHeadingController_Start(relative_heading_controller_t *control,
...
void RelativeHeadingController_Cancel(relative_heading_controller_t *control, relative_yaw_end_reason_t reason)
{
    if (control != 0)
    {
        control->active     = 0U;
...
uint8_t RelativeHeadingController_Update(relative_heading_controller_t *control,
                                         float                          yaw_deg,
                                         float                          yaw_rate_dps,
                                         uint32_t                       now_ms,
                                         float                         *angular_z)
{
    ...
    {
        RelativeHeadingController_Cancel(control, RELATIVE_YAW_END_TIMEOUT);
        return 0U;
    }
    ...
    if (RelativeHeadingController_Abs(error_deg) <= RELATIVE_YAW_TOLERANCE_DEG
        && RelativeHeadingController_Abs(yaw_rate_dps) <= RELATIVE_YAW_SETTLE_RATE_DPS)
    {
        ...
        {
            RelativeHeadingController_Cancel(control, RELATIVE_YAW_END_COMPLETED);
            return 0U;
        }
    }
    else
