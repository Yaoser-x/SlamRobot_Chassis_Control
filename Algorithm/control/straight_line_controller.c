...
static float InterpolateTrim(float speed_mps, float low_trim_mps, float high_trim_mps)
{
    ...
    return low_trim_mps
           + (high_trim_mps - low_trim_mps) * (speed_mps - STRAIGHT_TRIM_LOW_SPEED_MPS)
...
void StraightLineController_Init(straight_line_controller_t *controller)
{
    if (controller != NULL)
    {
...
void StraightLineController_Reset(straight_line_controller_t *controller)
{
    StraightLineController_Init(controller);
}

straight_line_controller_result_t StraightLineController_Step(straight_line_controller_t              *controller,
                                                              const straight_line_controller_params_t *params,
                                                              const straight_line_controller_input_t  *input)
{
    straight_line_controller_result_t result = {0};
    float                             dt_s;
    float                             speed_mps;
    ...
    {
        StraightLineController_Reset(controller);
        return result;
    }

    reset_reference =
        (uint8_t)((controller->active == 0U) || (controller->direction != direction)
                  || (controller->source != input->source) || (controller->generation != input->generation));
    ...
    if (direction > 0)
    {
        result.trim_correction_mps =
            InterpolateTrim(speed_mps, params->trim_forward_015_mps, params->trim_forward_030_mps);
    }
    else
    {
        result.trim_correction_mps =
            InterpolateTrim(speed_mps, params->trim_reverse_015_mps, params->trim_reverse_030_mps);
    }

    wheel_valid = (uint8_t)((input->left_speed_valid != 0U) && (input->right_speed_valid != 0U));
    if (wheel_valid != 0U)
    {
        result.wheel_correction_mps = params->wheel_coupling_gain * (input->actual_left_mps - input->actual_right_mps);
    ...
    else
    {
        ...
        candidate_integral_deg_s = ClampFloat(controller->heading_integral_deg_s + result.heading_error_deg * dt_s,
    ...
    unclamped_correction_mps = result.trim_correction_mps + result.wheel_correction_mps + result.heading_correction_mps;
    result.total_correction_mps = ClampFloat(unclamped_correction_mps, -correction_limit_mps, correction_limit_mps);
    result.correction_clamped   = (uint8_t)(fabsf(unclamped_correction_mps - result.total_correction_mps) > 0.000001f);
